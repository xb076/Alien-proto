...
namespace Alien {
	...
	static GLenum ShaderTypeFromString(const std::string& type)
	{
		...
		if (type == "vertex")
			...
			ALIEN_CORE_ASSERT(false, "Unkown Shader Type");
	...
	void OpenGLShader::UploadUniformFloat3(const std::string& name, const glm::vec3& value)
	{
		...
		if (location != -1)
			...
			ALIEN_CORE_ERROR("Uniform Float3 '{0}' not found!", name);
	...
	std::string OpenGLShader::ReadFile(const std::string& filepath)
	{
		...
		{
			ALIEN_CORE_ERROR("OpenGLShader could not open file '{0}'", filepath);
		}
	...
	std::unordered_map<GLenum, std::string> OpenGLShader::PreProcess(const std::string& source)
	{
		...
		while (pos != std::string::npos)
		{
			...
			ALIEN_CORE_ASSERT(eol != std::string::npos, "Syntax Error");
			size_t begin = pos + typeTokenLength + 1;
			std::string type = source.substr(begin, eol - begin);
			ALIEN_CORE_ASSERT(ShaderTypeFromString(type), "Invalid shader type specified");
	...
	void OpenGLShader::Compile(const std::unordered_map<GLenum, std::string> shaderSources)
	{
		...
		GLuint program = glCreateProgram();
		ALIEN_CORE_ASSERT(shaderSources.size() <= 2, "Only support 2 shaders.")
		...
		for (auto& kv : shaderSources)
		{
			...
			if (isCompiled == GL_FALSE)
			{
				...
				ALIEN_CORE_ERROR("{0}", infoLog.data());
				ALIEN_CORE_ASSERT(false, "Shader compilation failure!");
		...
		if (isLinked == GL_FALSE)
		{
			...
			ALIEN_CORE_ERROR("{0}", infoLog.data());
			ALIEN_CORE_ASSERT(false, "Shader link failure!");
